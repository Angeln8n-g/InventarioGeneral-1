...
export async function POST(request: NextRequest) {
  try {
    return await withAuth(request, async (authContext) => {
      ...
      const { qr_code, quantity, notes } = body
        ...
        return NextResponse.json(
          {
            error: {
              code: ERROR_CODES.VALIDATION_ERROR,
              message: `Insufficient stock. Available: ${stock.current_quantity}, Requested: ${quantity}`,
              timestamp: new Date().toISOString(),
            },
          },
          { status: 400 }
      ...
      try {
        await auditLogOperations.create({
          ...
          new_values: { current_quantity: newQuantity, quantity_consumed: quantity },
      ...
      return NextResponse.json({
        ...
        message: `Successfully consumed ${quantity} ${stock.unit_of_measure || 'units'} of ${stock.item_type.name}`,
      })
    })
  } catch (error: unknown) {
