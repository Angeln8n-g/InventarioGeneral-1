    ...
    return await withPermission(request, PERMISSIONS.USERS_VIEW_ALL, async () => {
      ...
      stockData?.forEach((stock: any) => {
        if (stock.current_quantity <= stock.minimum_threshold) {
          alerts.push({
            ...
            title: `Stock Crítico: ${stock.item_types.name}`,
      ...
      overdueLoans?.forEach((loan: any) => {
        ...
        alerts.push({
          ...
          title: `Préstamo Vencido: ${loan.users.username}`,
          message: `${loan.tool_instances.item_types.name} - Vencido hace ${daysOverdue} días`,
